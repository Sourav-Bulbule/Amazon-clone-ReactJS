...
const ProductDetails = ({product, ratings}) => {
  return (
    <div className="mb-1">
      ...
      {
        ratings && <div className="text-sm xl:text-base  mb-1"><ProductRatings avgRating={product.avgRating} ratings={product.ratings} /></div>
      }
      
      <div className="text-xs xl:text-sm font-bold mb-1">{product.attribute}</div>
      <div className="mb-2"><ProductBadge badge={product.badge}/>
      </div>
    </div>
  )
}
